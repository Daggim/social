  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (req.isAuthenticated()) {
        const feeds = await NewsFeed.find({ follower: req.user._id })
          .sort({ createdAt: -1 })
          .populate({
            path: "post",
            populate: {
        ...
        const filteredFeed = feeds
          .filter((feed) => {
            if (feed.post) {
              return (
                feed.post.privacy === "follower" ||
                feed.post.privacy === "public"
              );
            }
          ...
          .map((feed) => {
            ...
            const isOwnPost =
              feed.post._author_id.toString() === req.user._id.toString();
        ...
        const feeds = await Post.find({ privacy: EPrivacy.public })
          .sort({ createdAt: -1 })
          .populate({
            path: "author likesCount commentsCount",
            select: "profilePicture username fullname",
