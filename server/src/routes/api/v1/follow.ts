...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      bulk
        .find({ _user_id: Types.ObjectId(follow_id) })
        .upsert()
        .updateOne({
          $addToSet: {
      ...
      const doc = await Follow.aggregate([
        ...
        },
        {
          $group: {
            _id: "$_id",
            following: {
              $push: { user: "$user", isFollowing: "$isFollowing" },
            },
          },
        },
        {
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await User.findOne({ username });
      if (!user)
        return next(new ErrorHandler(400, `No ${username} user found.`));
