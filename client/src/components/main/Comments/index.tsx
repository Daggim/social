...
const Comments: React.FC<IProps> = (props) => {
  ...
  const fetchComment = async (params: IFetchParams) => {
    try {
      setIsLoading(true);
      const { comments: fetchedComments, commentsCount } = await getComments(
        postID,
        params
      );
      ...
      if (didMount) {
        setOffset(offset + 1);
        setComments({
          items: [...fetchedComments.reverse(), ...comments.items],
          commentsCount,
        });
        setIsLoading(false);
      }
  ...
  const handleSubmitComment = async (
    ...
    if (e.key === "Enter" && commentBody) {
      try {
        ...
        const comment = isUpdating
          ? await updateComment(targetID, commentBody)
        ...
        if (didMount) {
          if (isUpdating) {
            ...
            setComments({
              commentsCount: comments.commentsCount + 1,
  ...
  const deleteSuccessCallback = (commentID: string) => {
    ...
    if (didMount) {
      ...
      setComments({
        ...
        items: filteredComments as IComment[],
      });
    }
  };
      ...
      <div className="rounded-b-md border-t border-gray-200 dark:border-gray-800">
          ...
          <TransitionGroup component={null}>
            {comments.items.map((comment: IComment) => (
              <CSSTransition timeout={500} classNames="fade" key={comment.id}>
                <div className="flex py-2 items-start" key={comment.id}>
                  <Link
        ...
        {isUpdating && (
          <div className="flex items-center justify-between mt-4">
            <span className="text-xs ml-14 text-gray-400">
              Updating Comment. Press Esc to cancel
            </span>
            <span
        ...
        {isCommentVisible && (
          <div
            className={`flex items-center py-4 px-2 ${
              isUpdating && "bg-yellow-100 dark:bg-indigo-1100 rounded-2xl"
            }`}
          >
